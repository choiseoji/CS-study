...
> 큐는 First-In-First-Out 의 자료구조의 형태이며 마치 프린트를 요청하면 요청한 순서대로 프린트가 출력되는 형태와 같습니다.

> 우선 순위 큐는 큐에서 우선 순위에 따라서 나가는 순서가 정해지는 큐로, 같은 우선순위라면 먼저 들어오는 값이 나가게 된다는 점에서 FIFO를 따르지만 우선 순위가 높은 값이 먼저 나가는 특이한 자료 구조입니다. 이를 구현하기 위해 가장 대표적이고 일반적인 것이 heap 자료구조입니다.
