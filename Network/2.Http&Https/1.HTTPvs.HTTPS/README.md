...
> 둘 다 클라이언트와 서버 간 통신 규칙입니다. 요청과 응답을 통해 효율적으로 리소스를 교환하는 프로토콜입니다. 그러나 다른 점은 HTTPS는 HTTP에 TSL 을 결합해 인증서와 암호화를 통해 제3자가 중간에 도청을 할 수 없도록 Secure 이 추가된 것 입니다. 이외의 차이점은 HTTP는 80, HTTPS 는 443번 포트이고 HTTP는 애플리케이션, HTTPS 는 TSL 을 사용하기에 전송 계층의 통신이라는 점이 차이점입니다.
